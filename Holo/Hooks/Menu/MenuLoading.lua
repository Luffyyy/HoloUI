...
function LevelLoadingScreenGuiScript:init(scene_gui, res, progress, base_layer)
	local file = io.open("mods/saves/HoloHud_Options.txt", "r")
	local HoloSettings = {}
	if file then
		HoloSettings = json.decode(file:read("*all"))
		file:close()
	end 
	if HoloSettings.Loading == nil then
		HoloSettings.Loading = true
	end
	if HoloSettings.Base.Menu and HoloSettings.Loading then
		...
		local brief_text = background_safepanel:text({
			y = 2,
			text = arg.load_level_data.level_data.briefing_string,
			font = "fonts/font_large_mf",
			font_size = 16,
			align = "right",
	        halign="grow",
		...
		if HoloSettings.Colors.Main then
			colors[1] = colors[math.min(HoloSettings.Colors.Main + 1, #colors)]
			line:set_color(colors[math.min(HoloSettings.Colors.Main + 1, #colors)])			
		end 
 		if HoloSettings.Colors.MenuBackground then			
 			level_image:set_color(colors[HoloSettings.Colors.MenuBackground])			
 		end
