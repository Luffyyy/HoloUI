...
function HUDMissionBriefing:init(hud, workspace)
	...
	self._ready_slot_panel:set_bottom(self._foreground_layer_one:h() + 10)
	...
	for i = 1, 4 do
		...
		local slot_panel = self._ready_slot_panel:panel({
 		...
 		local bg = slot_panel:rect({
			name = "bg",
			color = Color.black,
			layer = -2,
			alpha = 0.3,
		})
		local line = slot_panel:rect({
			name = "line",
			color = Holomenu_color_tab_highlight,
			layer = -1,
			h = 2
		}):set_bottom(bg:bottom())
	...
	if contact_pattern then
		self._backdrop:set_pattern(contact_pattern, 0.1, "normal")
	end
	local padding_y = 60
	...
	local line = self._job_schedule_panel:rect({
		h = 2,
		color = Holomenu_color_tab_highlight,
		x = 1.1,
	})
	...
	local day_text = self._job_schedule_panel:text({
		...
		text = utf8.to_upper(managers.localization:text("menu_day_short", {
			day = tostring(managers.job:current_stage()) .. " / " .. num_stages 
		})),
		align = "center",
		vertical = "center",
		font_size = content_font_size,
	...
	ghost:set_center(day_text:right(), day_text:center_y())
	line:set_top(day_text:center_y() + day_text:h() * 0.32)
	local ghost_visible = managers.job:is_job_stage_ghostable(managers.job:current_real_job_id(), i)
	...
	self._job_schedule_panel:set_center_y(job_overview_text:center_y())
...
function HUDMissionBriefing:on_whisper_mode_changed()
	if alive(self._job_schedule_panel) then
		...
		local ghost_icon = self._job_schedule_panel:child("ghost")
