...
if not Holo.setup then
	...
	if Holo.options.Progress_active_color == nil then
	Holo.options.Progress_active_color = 1
	Holo:Save()
	end
	if Holo.options.Progress_invalid_color == nil then
	Holo.options.Progress_invalid_color = 7
	Holo:Save()
	end	
	...
	if Holo.options.hudline_enable == nil then
	Holo.options.hudline_enable = false
	Holo:Save()
	end
	...
	if Holo.options.Line_color == nil then 
	Holo.options.Line_color = 11
	Holo:Save()
	end
	...
	if Holo.options.Old_radial == nil then 
	Holo.options.Old_radial = false
	Holo:Save()
	end
	...
	if Holo.options.HealthNum_style == nil then 
	Holo.options.HealthNum_style = 1
	Holo:Save()
	end
	...
	if Holo.options.totalammo_enable == nil then 
	Holo.options.totalammo_enable = false
	Holo:Save()
	end
	...
	if Holo.options.HealthNum_negative == nil then 
	Holo.options.HealthNum_negative = 9
	Holo:Save()
	end
	Holo:Load()
	Holo.setup = true
...
Hooks:Add("LocalizationManagerPostInit", "LocalizationManagerPostInit_Holo", function( loc )
	loc:load_localization_file( Holo.mod_path .. "/loc/en.txt")
	loc:load_localization_file( Holo.mod_path .. "commands.txt")
end)
	...
	MenuCallbackHandler.HudBox_enable = function(self, item)
	    ...
	    MenuCallbackHandler.totalammo_enable = function(self, item)
		Holo.options.totalammo_enable = (item:value() == "on" and true or false)
		Holo:Save()
	end
	    ...
	    MenuCallbackHandler.Old_radial = function(self, item)
		Holo.options.Old_radial = (item:value() == "on" and true or false)
		Holo:Save()
		if item:value() == "on" then
		if managers.hud ~= nil then
		managers.hud._teammate_panels[HUDManager.PLAYER_PANEL]:set_old()
		end	
		Holo:Replace(Holo.mod_path .."Hud/Textures/HealthHoloBlue.texture", "hud_health.texture")	
		Holo:Replace(Holo.mod_path .."Hud/Textures/ShieldHoloBlue.texture", "hud_shield.texture")	
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/radial_bg_old.texture", "hud_radialbg.texture")		
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/radial_rim_old.texture", "hud_radial_rim.texture")		
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/swansong_old.texture", "hud_swansong.texture")		
        else
        if managers.hud ~= nil then
        managers.hud._teammate_panels[HUDManager.PLAYER_PANEL]:set_new()
        end
        Holo:Replace(Holo.mod_path .."Hud/Textures/Health"..Tex_Colors_Dir[Holo.options.Health_Color], "hud_health.texture")		
        Holo:Replace(Holo.mod_path .."Hud/Textures/Shield"..Tex_Colors_Dir[Holo.options.Shield_Color], "hud_shield.texture")	
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/radial_bg.texture", "hud_radialbg.texture")		
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/radial_rim.texture", "hud_radial_rim.texture")		
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/swansong.texture", "hud_swansong.texture")		
		end
	end
		MenuCallbackHandler.Progress_active_color = function(self, item)
		Holo.options.Progress_active_color = item:value()
		Holo:Save()
		Holo:Replace(Holo.mod_path .."Hud/Textures/Progress"..Tex_Colors_Dir[item:value()], "hud_progress_active.texture")		
	end
		MenuCallbackHandler.Progress_invalid_color = function(self, item)
		Holo.options.Progress_invalid_color = item:value()
		Holo:Save()
		Holo:Replace(Holo.mod_path .."Hud/Textures/Progress"..Tex_Colors_Dir[item:value()], "hud_progress_invalid.texture")		
	end
		MenuCallbackHandler.Health_Color = function(self, item)
		if not Holo.options.Old_radial then
        ...
        Holo:Replace(Holo.mod_path .."Hud/Textures/other/swansong.texture", "hud_swansong.texture")				
		 ...
		 local msg = QuickMenu:new( "Info", "To change health and shield color please turn off old radial health option.", {} )
         msg:Show()
	end
	end
		...
		MenuCallbackHandler.hudline_enable = function(self, item)
		Holo.options.hudline_enable = (item:value() == "on" and true or false)		
		Holo:Save()
	end
		...
		MenuCallbackHandler.HealthNum_style = function(self, item)
		Holo.options.HealthNum_style = item:value()		
		Holo:Save()
	end
		...
		MenuCallbackHandler.Line_color = function(self, item)
		Holo.options.Line_color = item:value()
		Holo:Save()
	end
		...
		MenuCallbackHandler.HealthNum_negative = function(self, item)
		Holo.options.HealthNum_negative = item:value()
		Holo:Save()
	end
		...
		local Frame_color = { ["Frame_color"] = true, ["Line_color"] = true, }
		local Shield_Color = { ["Shield_Color"] = true }
		local Progress_invalid_color = { ["Progress_invalid_color"] = true }
		local Old_radial = { ["Old_radial"] = true }
		MenuHelper:ResetItemsToDefaultValue( item, Old_radial , false )
		...
		MenuHelper:ResetItemsToDefaultValue( item, Progress_invalid_color , 7 )
	end 
		...
		MenuHelper:AddToggle({
			...
			MenuHelper:AddToggle({
			id = "totalammo_enable",
			title = "totalammo_enable_title",
			desc = "totalammo_enable_desc",
			callback = "totalammo_enable",
			icon_by_text = false,
			disabled_color = Color.black,
			menu_id = Holo.options_menu,
			value = Holo.options.totalammo_enable,
			priority = 1,
		})	
			MenuHelper:AddToggle({
			id = "hudline_enable",
			title = "hudline_enable_title",
			desc = "hudline_enable_desc",
			callback = "hudline_enable",
			icon_by_text = false,
			disabled_color = Color.black,
			menu_id = Holo.options_menu,
			value = Holo.options.hudline_enable,
			priority = 0,
		})		
			...
			id = "HealthNum_style",
			title = "HealthNum_style_title",
			desc = "HealthNum_style_desc",
			callback = "HealthNum_style",
			menu_id = Holo.options_menu,
			value = Holo.options.HealthNum_style,
			items = HP_Styles,
			priority = -3,
		})  
        ...
        --Colors menu		
			...
			title = "Progress_active_title",
			desc = "Progress_active_desc",
			callback = "Progress_active_color",
			menu_id = Holo.options_colors,
			value = Holo.options.Progress_active_color,
			items = Radial_Colors,
			priority = 12,
		})   
			MenuHelper:AddMultipleChoice({
			id = "Progress_invalid_color",
			title = "Progress_invalid_title",
			desc = "Progress_invalid_desc",
			callback = "Progress_invalid_color",
			menu_id = Holo.options_colors,
			value = Holo.options.Progress_invalid_color,
			items = Radial_Colors,
			priority = 11,
		})   		
			MenuHelper:AddMultipleChoice({
		...
		MenuHelper:AddMultipleChoice({
			...
			callback = "Line_color",
			menu_id = Holo.options_colors,
			value = Holo.options.Line_color,
			items = HoloColors,
			priority = -2,
		})  
			MenuHelper:AddToggle({
			id = "Old_radial",
			title = "Old_radial_title",
			desc = "Old_radial_desc",
			callback = "Old_radial",
			icon_by_text = false,
			disabled_color = Color.black,
			menu_id = Holo.options_colors,
			value = Holo.options.Old_radial,
			priority = -3,
		})  
		    MenuHelper:AddDivider({
            id = "divider2",
        ...
        --Text menu
			...
			priority = 11,
		   	...
		   	MenuHelper:AddMultipleChoice({
			id = "HealthNum_negative",
			title = "HealthNum_negative_title",
			desc = "HealthNum_negative_desc",
			callback = "HealthNum_negative",
			menu_id = Holo.options_text,
			value = Holo.options.HealthNum_negative,
			items = HoloColors,
			priority = 10,
		})  
			MenuHelper:AddMultipleChoice({
...
Hooks:Add("MenuManagerBuildCustomMenus", "HolohudOptions", function(menu_manager, nodes)
		...
		MenuHelper:AddMenuItem( MenuHelper.menus[Holo.options_menu], Holo.options_colors , "Options_colors_title", "Options_colors_desc", 17)
		
		nodes[Holo.options_text] = MenuHelper:BuildMenu(Holo.options_text )
		MenuHelper:AddMenuItem( MenuHelper.menus[Holo.options_menu], Holo.options_text , "Options_text_title", "Options_text_desc", 18)
	end)
end